    ...
    "get_all_feature_values":function(){
      if (Session.get("feature") != undefined){
        ...
        songs.forEach(function(song){
          //console.log(song);
            features[ind] = {
              artist:song.metadata.tags.artist,//json location for this piece of data
              title:song.metadata.tags.title, 
              //now we get the value of that feature
              value:song[Session.get("feature")["type"]][Session.get("feature")["name"]]
            };
            ind ++;
        })
        return features;//return tha value for that feature
      }
      else {//if an empty value return an empty array
        return [];
      }
    }
  })
...
////////////////////////////
  ...
  Template.song_viz_controls.events({
    ...
    //event handler for when the user clicks on the
    //line graph button
    "click .js-show-linegraph":function(event) {
      event.preventDefault();
      initLineVis();
    }, 
  }); 
...
}//its a network to show the connectedness of the dot


//function that creates a line graph visualisation
